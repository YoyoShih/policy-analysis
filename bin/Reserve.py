...
with xw.App(visible = True, add_book = False) as app:
    ...
    try:
        ...
        try:
            sheet = wb.sheets('Reserve')
            sheet.clear()
        except:
            sheet = wb.sheets.add('Reserve')
    except:
        wb = app.books.add()
        sheet = wb.sheets[0]
        sheet.name = 'Reserve'
    ...
    cols = ['t', 'x', 'q', 'L', 'Bnf', 'Bnf PV', 'NP', 'NP PV', 'VX']
    sheet.range('D1:L1').value = cols
    L = 1
    ...
    for t in range(age_end - insured_age + 1):
        ...
        NP_PV = NP * a(x, t)
        VL = Bnf_PV - NP_PV
        sheet.range(f'D{t + 2}:L{t + 2}').value = [t, x, q, L, Bnf, Bnf_PV, NP, NP_PV, VL]
        L -= d
